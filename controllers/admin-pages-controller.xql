...
declare function admin-pages:projects($node as node(), $model as map(*)) as map(*) {
    config:trash($node, $model),
    if (index-of(xmldb:get-user-groups(xmldb:get-current-user()), "dba")>0) then (
...
declare function local:activate-projects-controller() as empty() {
...
declare function local:activate-scheduler() {
    let $collection := $config:ediarum-db-path||"/routinen"
    let $resource := "temp.xml"
    let $store := xmldb:store($collection, $resource, config:get-existdb-config())
    let $conf := doc($store)
    let $scheduler-job :=
        <job type="user" name="ediarumScheduler" xquery="{$config:ediarum-db-path}/routinen/scheduler.xql" cron-trigger="0 * * * * ?"/>
...
declare function local:change-ports($port, $ssl-port) {
    ...
    let $update-port-ssl := replace($update-port,'<SystemProperty name="jetty\.port\.ssl" default="\d+"/>',
        concat('<SystemProperty name="jetty.port.ssl" default="',$ssl-port,'"/>'))
    let $conf := $update-port-ssl

    let $binary := util:string-to-binary($conf)
...
declare function local:create-new-project($p) {
    ...
    xmldb:create-collection("/db/projects",$p),
    ...
    return (
        ...
        config:mkcol-in-project($p, "", "schemata", "dba", "rwxrwx---"),
        config:mkcol-in-project($p, "schemata", "archive", "dba", "rwxrwx---"),
        ...
        (: Die Benutzerberechtigungen für die Collections werden eingerichtet. :)
        ...
        (:local:copy_file_to_system_config($p, concat($config:ediarum-db-path,"/setup"), "data", "permissions.xconf", "collection.xconf"),:)
        (:let $permission := doc("/db/system/config"||config:project-collection-path($p, "data")||"/collection.xconf")
        let $parameter := <parameter xmlns="http://exist-db.org/collection-config/1.0" name="group-name" value="{$nutzer-group}" />
        return
...
declare function local:delete-project($p) {
    ...
    then (
        ...
        xmldb:remove(concat("/db/projects/",$p)),
        xmldb:remove(concat("/db/system/config/db/projects/",$p)),
        let $nutzer-group := concat($p, '-nutzer')
...
declare function local:projects-controller-is-running() {
    ...
    let $statusCode := httpclient:head(xs:anyURI("http://"||request:get-server-name()||":"||request:get-server-port()||"/exist/projects/"), false(), ())/@statusCode/string()
    let $projects-controller-running := exists(index-of($valid-status-codes, $statusCode))
    return
